...
public class FrmListeDeclarationDelaisPaiement : RibbonForm, IGridViewForm, IDeclarationDelaisPaiementNotify, IGridLayoutCustomizable
{
	...
	private List<DeclarationDelaisPaiementView> _declarationCollection;

	private readonly LayoutController _layoutController;

	private readonly IFormFactory _uiFactory;

	private GridViewMenu _dataRowMenu;

	private const string CGridName = "La liste des modèles [Liste des déclarations DP]";
	...
	public GridViewMenu ViewMenu => _dataRowMenu;

	public Guid GridGuid { get; private set; }

	public int? AppliedLayoutNo { get; set; }

	public string GridName => "La liste des modèles [Liste des déclarations DP]";

	public GridLayoutCutomizationMenu LayoutMenu { get; set; }

	private FrmListeDeclarationDelaisPaiement()
	{
		InitializeComponent();
	...
	public FrmListeDeclarationDelaisPaiement(ListDeclarationDelaisPaiementController controller, LayoutController layoutController, IFormFactory uiFactory)
		: this()
	{
		...
		_uiFactory = uiFactory ?? throw new ArgumentNullException("uiFactory");
		Initialize();
	}

	protected override void OnShown(EventArgs e)
	{
		LayoutMenu = new GridLayoutCutomizationMenu(_layoutController, _uiFactory, this);
		GridGuid = (Guid)gv.Tag;
	...
	private void AjouterDeclaration(object sender, EventArgs e)
	{
		...
		DeclarationDelaisPaiementView view = _controller.InitView();
		((IParentFormDeclaration)base.MdiParent).OpenDeclarationDelaisPaiement(view);
	}

	private void DeleteDeclaration(object sender, EventArgs e)
	{
		if (!(gv.GetFocusedRow() is DeclarationDelaisPaiementView view))
		{
			return;
		}
	...
	private void OpenDeclaration(object sender, RowClickEventArgs e)
	{
		...
		if (e.Button == MouseButtons.Left && e.Clicks == 2 && gv.GetFocusedRow() is DeclarationDelaisPaiementView view && base.MdiParent is IParentFormDeclaration parentFormDeclaration)
		{
			parentFormDeclaration.OpenDeclarationDelaisPaiement(view);
		}
	}
	...
	public void DeclarationDelaisPaiementChanged(DeclarationDelaisPaiementChangedArgs e)
	{
		...
		case TypeAction.Ajout:
		{
			...
			{
				NotifyDeclarationDelaisPaiementChangedAjout(e);
			};
			BeginInvoke(method3);
			break;
		}
		case TypeAction.Suppression:
		{
			...
			{
				NotifyDeclarationDelaisPaiementChangedSupp(e);
			};
			BeginInvoke(method2);
			break;
		}
		case TypeAction.Modification:
		{
			...
			{
				NotifyDeclarationDelaisPaiementChangedModify(e);
			};
			BeginInvoke(method);
			break;
		}
	...
	private void NotifyDeclarationDelaisPaiementChangedAjout(DeclarationDelaisPaiementChangedArgs e)
	{
		DeclarationDelaisPaiementView declarationDelaisPaiementView = _controller.Get(e.EntityNo);
		if (declarationDelaisPaiementView != null)
		{
			_declarationCollection.Add(declarationDelaisPaiementView);
			GridviewSelectionChanged(null, null);
			_pLinqEntet.Refresh();
		}
	}

	private void NotifyDeclarationDelaisPaiementChangedModify(DeclarationDelaisPaiementChangedArgs e)
	{
		DeclarationDelaisPaiementView declarationDelaisPaiementView = _declarationCollection.SingleOrDefault((DeclarationDelaisPaiementView x) => x.No == e.EntityNo);
		if (declarationDelaisPaiementView == null)
		{
			return;
		}
		DeclarationDelaisPaiementView declarationDelaisPaiementView2 = _controller.Get(e.EntityNo);
		if (declarationDelaisPaiementView2 == null)
		{
			return;
		}
		int num = _declarationCollection.IndexOf(declarationDelaisPaiementView);
		if (num < 0)
		{
			return;
		}
		...
		try
		{
			_declarationCollection[num] = declarationDelaisPaiementView2;
			GridviewSelectionChanged(null, null);
			_pLinqEntet.Refresh();
		}
		catch (Exception ex)
	...
	private void NotifyDeclarationDelaisPaiementChangedSupp(DeclarationDelaisPaiementChangedArgs e)
	{
		DeclarationDelaisPaiementView declarationDelaisPaiementView = _declarationCollection.SingleOrDefault((DeclarationDelaisPaiementView x) => x.No == e.EntityNo);
		if (declarationDelaisPaiementView != null)
		{
			_declarationCollection.Remove(declarationDelaisPaiementView);
			GridviewSelectionChanged(null, null);
			_pLinqEntet.Refresh();
		}
	}
	...
	{
		this.ApplyModelLayout(layout, btnLigneFilter, bbiZoneGroupement, bbiZoneRecherche);
	}
	...
	private void Initialize()
	{
		...
		_ = persistentRepository.Items[0];
		...
		gv.Tag = new Guid("{F9634356-4D23-4371-ADB4-225E403E6ADD}");
