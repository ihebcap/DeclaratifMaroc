...
**Preuve (`git show 842f9ef --stat`)** : le commit `842f9ef` modifie
`Declaration.Selection/SelectionExpliqueeService.cs` (143 lignes) et ajoute
`Declaration.Orchestration.Tests/Task197ExemptionFiltreSelectionTests.cs` (141 lignes) —
exactement les fichiers attendus par cette tâche. Le commit `81358ad`, lui, ne contient
...
### Suite complète (remplace la vérification partielle ci-dessus)

### Suite complète (`dotnet test DeclarationTVA.slnx --no-build`, exécutée le 2026-08-07 depuis `d:\_vibe\GRF`)
